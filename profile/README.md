...
### 2. 분석 기준 및 도출 인사이트
* **분석 기준:**
    * 표준화 지표 도입: 매물마다 제각각인 면적을 객관적으로 비교하기 위해, 단순 보증금 총액이 아닌 **'평당 보증금(3.3㎡당 가격)'**을 핵심 지표로 산출하여 가격 거품을 제거함.
    * 데이터 정제: 분석의 정확도를 위해 임대면적이나 평수가 0인 이상치(Outlier) 및 결측치를 제거하고, 마포구 내 실제 '전세' 거래 데이터만 필터링하여 활용.
* **인사이트 (Insights):**
    * 📈 **분석 결과:**
       * 지역별 가격 편차: Box Plot 분석 결과, 일부 동네는 상자(Box)의 길이가 길게 나타나 동일한 동네 내에서도 가격 차이가 크다는 점(저렴한 매물 발굴 기회 존재)을 확인.
       * 평균 회귀: Scatter Plot 분석 시, 대다수 매물이 평균 시세선 근처에 분포하나, 명확하게 시세선 하단에 위치한 저평가 구간이 존재함.
    * 💡 **솔루션:**
